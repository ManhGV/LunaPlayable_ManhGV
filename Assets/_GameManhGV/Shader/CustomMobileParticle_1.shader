...
Shader "Horus/Mobile/Particles"
{
    ...
    {
        v2f o;
        float3 pullPos = mul(unity_ObjectToWorld, v.position);
        float3 camDirection = normalize(_WorldSpaceCameraPos - pullPos);
        pullPos += camDirection * _Pull;
        o.position = mul(UNITY_MATRIX_VP, float4(pullPos, 1));
        o.texcoord = TRANSFORM_TEX(v.texcoord, _MainTex);
        o.dissolveUV = v.dissolveUV * _DissolveTexture_ST.xy + _DissolveTexture_ST.zw + _DissoveTextureVelocity.xy * _Time.y;
        o.color = v.color;
        #if _ENABLEFOG_ON
